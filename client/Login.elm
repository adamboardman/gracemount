...
import Loading
import Types exposing (LoginForm, Model, Msg(..), Problem(..), Session, UserPermissionsType(..), ValidatedField(..))
...
userIsEditor model =
    loggedIn model && (model.loggedInUser.permissions == UserPermissionsEditor || model.loggedInUser.permissions == UserPermissionsAdmin)


userIsAdmin : Model -> Bool
userIsAdmin model =
    loggedIn model && model.loggedInUser.permissions == UserPermissionsAdmin
